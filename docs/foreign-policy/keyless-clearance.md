...
**openab already ships the reference implementation — `SsmSecurityStack` (#97):** GitHub App private keys live as **SSM SecureString** at `/openab/github/app/<app>/private-key`, each encrypted by a **per-App CMK**; each agent task role is granted `kms:Decrypt` on **only its own** key; and **EventBridge + SNS alert** on any read by a non-agent-role principal (`anything-but` the legit role ARNs). That is the scoped-vault pattern in production — reuse it to gate mint-time inputs. **[Today]**
